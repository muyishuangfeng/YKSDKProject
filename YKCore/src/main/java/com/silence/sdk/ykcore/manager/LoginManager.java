...
import com.silence.sdk.ykcore.model.Result;
    ...
    public static class LifecycleManager implements LifecycleObserver {
        ...
        private void prepareLogin(Activity activity, @Target.LoginTarget int target,
            ...
            if (!platform.isInstall(activity)) {
                listener.onState(originActivity.get(), LoginResult.failOf(target, YKGameError.make(YKGameError.CODE_NOT_INSTALL)));
            }
            //跳转
        ...
        private void postLogin(Activity activity) {
            ...
            if (currentTarget == -1) {
                mStateListener.onState(activity, LoginResult.
                        failOf(YKGameError.make(YKGameError.CODE_COMMON_ERROR, "login target error")));
                return;
            }
            //没有设置登录回调
            if (mStateListener == null) {
                mStateListener.onState(activity,
                        LoginResult.failOf(currentTarget,
                                YKGameError.make(YKGameError.CODE_COMMON_ERROR, "没有设置 login listener")));
                return;
            }
            //创建的 platform 失效
            if (GlobalPlatform.getCurrentPlatform() == null) {
                mStateListener.onState(activity,
                        LoginResult.failOf(currentTarget,
                                YKGameError.make(YKGameError.CODE_COMMON_ERROR, "创建的 platform 失效")));
                return;
            }
